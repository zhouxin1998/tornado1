...
class Session(object):
    def __init__(self, request_handler_obj):
        ...
        if not self.session_id:# session_id　不存在创建一个全局唯一的session_id
    ...
    def save(self):
        ...
        try:
            self.request_handler.redis.setex('sess_%s'%self.session_id,SESSION_EXPIRES_SECONDS,json_data)
        except Exception as e:
            logging.error(e)
            raise e
    def clear(self):
